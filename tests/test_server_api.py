...
def test_reset_run_outputs_clears_stale_verification_results(tmp_path):
    path = tmp_path / "verification_results.json"
    path.write_text('{"TS_F001_001": {"passed": true}}', encoding="utf-8")

    _reset_run_outputs(tmp_path)

    assert _read_json_file(path, None) == {}
...
def test_list_screenshot_items_keeps_realtime_mtime_order(tmp_path):
    step_9 = tmp_path / "TS_F001_002_成功_step_9.png"
    step_7_retry = tmp_path / "TS_F001_002_step_7_1.png"
    step_9.write_bytes(b"step9")
    step_7_retry.write_bytes(b"step7 retry")
    os.utime(step_9, (1000, 1000))
    os.utime(step_7_retry, (1001, 1001))

    items = _list_screenshot_items(tmp_path)

    assert [item["filename"] for item in items] == [
        "TS_F001_002_成功_step_9.png",
        "TS_F001_002_step_7_1.png",
    ]
